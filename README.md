...
CodePal 是一个面向 IDE / AI Agent 场景的统一监控面板。当前第一期优先聚焦 Codex，目标是在一个浮动窗口里集中查看 Codex 任务状态，减少在 Codex 和终端之间反复切换的成本。
...
- CodeBuddy / PyCharm 仍不在当前第一期交付承诺内，后续再扩展
...
- Codex 当前通过读取 `~/.codex/sessions/**/*.jsonl` 接入，不依赖独立 hook 配置
...
- `Codex`：直接读取 `~/.codex/sessions/**/*.jsonl`，当前不需要安装额外 hooks
